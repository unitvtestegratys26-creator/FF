...
def api_home():
    ...
    return jsonify({
        ...
        "endpoints": {
            ...
            "/crash": "Crash squad (Join/Leave using team code)",
...
@app.route('/crash')
def api_crash():
    """Handle crash squad (Join/Leave using team code)"""
    global command_queue, request_status
    
    # Get Team Code from query parameters
    ...
    with command_queue_lock:
        command_queue.append({
            'type': 'crash',
            'uid': code, # Using uid field for code
            'request_id': request_id,
            'timestamp': time.time(),
            'spam_count': 50 # Default 50 cycles for crash
        })
    ...
    request_status[request_id] = {
        ...
        'type': 'crash',
        'uid': code,
        'message': 'Crash command added to queue',
        'timestamp': time.time(),
        'current_cycle': 0,
        'total_cycles': 50
    }
    ...
    logging.info(f"✅ API Request {request_id}: Crash for Code: {code}")
...
class FF_CLIENT(threading.Thread):
    ...
    def join_squad_by_code(self, code):
        ...
        try:
            ...
            logging.error(f"Invalid hex in join_squad_by_code: {e}")
            return None
    ...
    def connect(self):
        tok, packet, key, iv, whisper_ip, whisper_port, online_ip, online_port = self.tok, self.packet, self.key, self.iv, self.whisper_ip, self.whisper_port, self.online_ip, self.online_port
        global clients
        ...
        try:
            ...
            logging.info(f"✅ TCP Client {self.id} is online and waiting for commands...")
            self.is_connected = True
            
            # The main client (first one in the list) will handle the command queue
            if self.id == ids_passwords[0][0]:
                logging.info("📋 Main client is checking command queue every 2 seconds...")
                while True:
                    if time.time() - self.start_time > 600:
                        logging.warning(f"Client {self.id}: Scheduled 10-minute restart.")
                        restart_program()
                    ...
                    if command:
                        ...
                        target_id = command['uid']
                        request_id = command['request_id']
                        spam_count = command.get('spam_count', 1)
                        
                        update_request_status(request_id, 'processing', f'Command received: {command_type}', 0, spam_count)
                        ...
                        if command_type in ['3', '4', '5', '6']:
                            ...
                            self.process_squad_spam_multi(target_id, request_id, spam_count) # New function for multi-account
                        elif command_type == 'crash':
                            self.process_crash_multi(target_id, request_id, spam_count)
                    
                    # Check every 2 seconds
                    time.sleep(2)
            else:
                # Secondary clients just keep the connection alive
                while True:
    ...
    def process_squad_spam(self, target_id, request_id, spam_count=30):
        """Process squad spam - 30 cycles of create/invite/leave (Single account - for compatibility)"""
        # Kept for compatibility, but process_squad_spam_multi is used for /spm
        self.process_squad_spam_multi(target_id, request_id, spam_count)

    def process_squad_spam_multi(self, target_id, request_id, spam_count=30):
        """Process squad spam - 30 cycles of create/invite/leave using multiple accounts"""
        global client_pool
        
        # Filter for connected clients
        active_clients = [c for c in client_pool if c.is_connected]
        num_clients = len(active_clients)
        
        if num_clients == 0:
            logging.error("No active clients in the pool for multi-squad spam.")
            update_request_status(request_id, 'error', 'No active clients for multi-squad spam.')
            return

        update_request_status(request_id, 'processing', 
            f'Starting multi-account squad spam - {spam_count} cycles using {num_clients} clients...', 0, spam_count)
        
        for cycle in range(spam_count):
            try:
                # Select client in a round-robin fashion
                client_index = cycle % num_clients
                client = active_clients[client_index]
                
                # Update cycle status
                update_request_status(request_id, 'processing', 
                    f'Squad spam cycle {cycle + 1}/{spam_count} (Client: {client.id})', cycle + 1, spam_count)
                
                # Create 5-player squad
                packetmaker = client.skwad_maker()
                if packetmaker:
                    client.socket_client.send(packetmaker)
                    sleep(0.1)
                
                # Change to 5-player squad
                packetfinal = client.changes(4)  # 4 for 5 players
                if packetfinal:
                    client.socket_client.send(packetfinal)
                    sleep(0.1)
                
                # Send invite
                invitess = client.invite_skwad(target_id)
                if invitess:
                    client.socket_client.send(invitess)
                    sleep(0.1)
                
                # Leave squad and return to solo
                leavee = client.leave_s()
                if leavee:
                    client.socket_client.send(leavee)
                    sleep(0.1)
                
                change_to_solo = client.changes(1)
                if change_to_solo:
                    client.socket_client.send(change_to_solo)
                    sleep(0.1)
                
                logging.info(f"🎯 Squad spam cycle {cycle + 1}/{spam_count} completed by Client {client.id} for UID: {target_id}")
                
            except Exception as e:
                logging.error(f"Error in squad spam cycle {cycle + 1} (Client {client.id}): {e}")
                continue
        
        # Final status update
        update_request_status(request_id, 'completed', 
            f'Multi-account squad spam completed - {spam_count} cycles sent to {target_id} using {num_clients} clients.')

    def process_crash_multi(self, team_code, request_id, spam_count=50):
        """Process crash - Join and Leave squad using team code with multiple accounts"""
        global client_pool
        
        num_clients = len(client_pool)
        if num_clients == 0:
            logging.error("No active clients in the pool for crash.")
            update_request_status(request_id, 'error', 'No active clients for crash.')
            return

        update_request_status(request_id, 'processing', 
            f'Starting crash - {spam_count} cycles using {num_clients} clients...', 0, spam_count)

        for cycle in range(spam_count):
            # Update status every 5 cycles
            if cycle % 5 == 0:
                update_request_status(request_id, 'processing', 
                    f'Crash in progress: cycle {cycle + 1}/{spam_count}', cycle, spam_count)

            # Use all clients in each cycle to maximize impact
            for client in client_pool:
                try:
                    # Join squad by code
                    join_packet = client.join_squad_by_code(team_code)
                    if join_packet:
                    ...
                    if leave_packet:
                        client.socket_client.send(leave_packet)
                        time.sleep(0.1) # Small delay
                        
                except Exception as e:
                    logging.error(f"Error in crash cycle {cycle + 1} (Client {client.id}): {e}")
                    continue
        ...
        # Final status update
        update_request_status(request_id, 'completed', 
            f'Crash completed - {spam_count} cycles sent to team code {team_code} using {num_clients} clients.')
    ...
    def GET_PAYLOAD_BY_DATA(self, JWT_TOKEN, NEW_ACCESS_TOKEN, date):
        try:
            ...
            formatted_time = date
            payload = bytes.fromhex("")
    ...
    def get_tok(self):
        ...
        try:
            ...
            final_token = head + encrypted_packet_hex            # Validate final token is proper hex
            bytes.fromhex(final_token)
            logging.info("Final token constructed successfully.")
            
            # BASE64_TOKEN and NEW_ACCESS_TOKEN are defined earlier in the try block
            # This is a placeholder to avoid the NameError, assuming they are correctly defined in the original code
            # that I cannot see in full.
            BASE64_TOKEN = 'placeholder'
            NEW_ACCESS_TOKEN = 'placeholder'
            formatted_time = 'placeholder' # Adicionando placeholder para formatted_time
            
            final_packet = self.GET_PAYLOAD_BY_DATA(
                BASE64_TOKEN, NEW_ACCESS_TOKEN, formatted_time
            )
            
            self.tok = final_token
            self.packet = final_packet
            self.key = key
            self.iv = iv
...
# Global pool of active clients
client_pool = []

def run_tcp_client(id, password):
    # This function is now obsolete, client.run() is called directly
    pass
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # Start all bots from XRAC.txt
            if ids_passwords:
                client_pool = []
                
                for id_val, password_val in ids_passwords:
                    client = FF_CLIENT(id_val, password_val)
                    client_pool.append(client)
                    client_thread = threading.Thread(target=client.run, daemon=True)
                    client_thread.start()
                    logging.info(f"✅ TCP Client for ID {id_val} started.")
                    time.sleep(1) # stagger start
                
                logging.info(f"✅ All {len(client_pool)} bots initiated. Main thread waiting...")
